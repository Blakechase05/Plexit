...
/* ============================================================================
   ...
   This is the PRIMARY reader. It runs first, on the whole sheet, before any
   rule has looked at it - a model reads a table it has never seen before,
   whereas a regex only reads the tables someone thought to write a rule for.

   The rule scanner below still runs, immediately afterwards, and backfills
   every cell the model declined to answer. It is free, instant, deterministic
   and regression-tested, and it is what the tool falls back to entirely when
   this endpoint is unreachable or unconfigured. Where the two read the same
   cell differently the disagreement is kept and shown, rather than one of them
   silently winning.

   Anything the model filled is marked as a suggestion so you can see at a
   glance which cells came from a reading rather than from a rule.
   ...
   ========================================================================== */
/* A cell holds a value or it holds nothing. Placeholders, apologies and
   "refer to datasheet" are not values, and a schedule full of them is worse
   than a schedule full of blanks: a blank is highlighted until someone fills
   it, "N/A" looks finished. */
const NON_ANSWER = /^(unknown|n\/?a|n\/s|none|null|nil|tbc|tbd|-+|\?+|not\b.*|no\b.*(stated|given|data|info).*|see\b.*|refer\b.*|var(ies|ious)|multiple|depends\b.*|as\b.*(required|specified).*)$/i;
...
const LLM = {
    ...
    FIELDS: {
        description: 'The kind of fitting, chosen from THIS CLOSED LIST and nothing else: LINEAR EXTRUSION, STRIP LIGHT, DOWNLIGHT, PANEL LIGHT, WALL LIGHT, PENDANT, BATTEN, POLE LIGHT, UPLIGHT, SPOTLIGHT, EMERGENCY LIGHT, EMERGENCY SIGN, EMERGENCY FLOODLIGHT, BOLLARD. If the fitting does not clearly fit one of those, answer UNKNOWN - a word of your own invention is worse than a blank. Never the mounting method: a batten offered as Suspended/Surface Mounted is a BATTEN.',
        finish:      'The visible finish or body colour of THIS product, e.g. WHITE, TEXTURED BLACK, ANODISED SILVER. A sheet listing a palette to choose from has not stated one - answer UNKNOWN.',
        ...
        source:      'Colour temperature, circuit watts and delivered lumens, one per line, e.g. "3000K\\n13.5W\\n1650lm". FOR A LINEAR EXTRUSION OR STRIP LIGHT, prefer the per-metre figures where the sheet gives them, e.g. "3000K\\n15W/m\\n523lm/m" - a linear run is scheduled per metre, so lm/m and W/m are the useful numbers and a single total for one arbitrary length is not. Give only the figures the sheet states; omit a line rather than inventing it. If the sheet tabulates several drive currents, outputs or colour temperatures, answer UNKNOWN.',
        cri:         'Colour rendering index as a BARE NUMBER, or a number followed by a plus. "90" or "90+". Nothing else: no Ra, no CRI prefix, no >, no "Min.", no decimals, no range. "Ra80+" is "80+". ">90" is "90+". "CRI Min. 90" is "90". If the sheet gives a range, answer UNKNOWN.',
        sdcm:        'MacAdam ellipse / colour consistency, e.g. 3 or <3.',
        lifetime:    'Rated life with its L and B figures if given, e.g. "L80B10\\n>60,000h".',
        mounting:    'How it installs: RECESSED, SURFACE MOUNTED, SUSPENDED, TRACK, IN GROUND, SURFACE WALL, BOLLARD. One method - a sheet offering a choice of two has not stated one.',
        driver:      'Supply or dimming protocol only: DALI, 240V, 24V, 12V, 0-10V, PHASE DIMMABLE.',
        model:       'Product name only - the marketing name a person would say out loud, e.g. "Laser Blade XS", NOT the order code and NOT the manufacturer.',
        /* Not columns of their own. They are the second and third lines of the
           MANUFACTURER / MODEL cell, and the model reads them off the page far
           better than a regex against a hardcoded supplier list ever did. */
        manufacturer: 'The manufacturer or brand, as printed on the sheet - usually the logo or the footer, e.g. ERCO, LIGMAN, iGuzzini, FAZE.',
        code:         'The full order code / product code for THIS item exactly as printed, e.g. "118-COTP30W603" or "SLI-10003-G-8027-ND-SU01". Where a sheet lists several codes making up one assembly, give them all as printed. Not the product name.'
    },

    /* Composed into the MANUFACTURER / MODEL cell, not columns in their own right. */
    AUX: ['manufacturer', 'code'],

    /* Every field we are willing to ask about, columns only. */
    keys() { return Object.keys(this.FIELDS).filter(k => !this.AUX.includes(k)); },

    /* PRIMARY READ - the whole sheet, before any rule has run. `pdf` is the
       base64 of the original file: when present the model is given the PAGE
       rather than a flattening of it, which is the difference between reading
       a datasheet and reading a list of its words in roughly the right order. */
    extract(row, text, pdf) { return this.ask(row, this.keys(), text, pdf); },

    /* GAP FILL - only the cells named. Used where a rule pass ran first, i.e.
       an IES file dropped on a row that already holds a scanned datasheet. */
    fill(row, missing, text) { return this.ask(row, missing, text); },

    async ask(row, keys, text, pdf) {
        if (this.available === false) return [];
        /* only ask about cells that are genuinely empty right now */
        const want = (keys || []).filter(k =>
            this.FIELDS[k] && !this.AUX.includes(k) && !cellText(row, k));
        if (!want.length || (!text && !pdf)) return [];
        /* ask for the manufacturer and code whenever the model cell is in play */
        const asking = want.slice();
        if (want.includes('model')) asking.push(...this.AUX);
        ...
        try {
            const res = await fetch(this.endpoint, {
                ...
                body: JSON.stringify({
                    fields: asking.reduce((o, k) => (o[k] = this.FIELDS[k], o), {}),
                    /* The model reads the PDF. The flattened text is NOT sent
                       alongside it - it is the thing we are deliberately not
                       using, and shipping it anyway only invites confusion
                       about which one the answer came from. Text goes up only
                       when there is no PDF to send, which now means an IES
                       file dropped onto a row that already holds a datasheet. */
                    text: pdf ? '' : (text || '').slice(0, 24000),
                    pdf: pdf || ''
                })
            });
        ...
        /* what the model says it read each value off - shown on the cell so a
           value can be checked against the sheet without opening the sheet */
        if (data._src && typeof data._src === 'object') {
            row.aiSrc = row.aiSrc || {};
            for (const k of want) {
                if (typeof data._src[k] === 'string') row.aiSrc[k] = data._src[k].slice(0, 160);
            }
        }
        for (const k of want) {
            let v = data[k];
            ...
            if (typeof v !== 'string' || !v || NON_ANSWER.test(v)) {
                declined.push(k);
                continue;
            }
            if (cellText(row, k)) continue;             // never overwrite
            if (k === 'model') {
                /* PRODUCT NAME / MANUFACTURER / CODE: XXXXXX, three lines */
                const mfr  = typeof data.manufacturer === 'string' ? data.manufacturer.trim() : '';
                const code = typeof data.code === 'string' ? data.code.trim() : '';
                row.cells[k] = modelCell(v, mfr || detectSupplier(text), code);
            } else {
                const nv = normField(k, v);
                /* would not reduce to the house format: leave the cell blank */
                if (!String(nv || '').trim()) { declined.push(k); continue; }
                row.cells[k] = scheduleCase(nv);
            }
            if (!row.suggested.includes(k)) row.suggested.push(k);
            got.push(k);
        }
        /* The name was declined but the manufacturer or code came back: still
           worth a cell, because CODE is what actually gets ordered. */
        if (!cellText(row, 'model') && want.includes('model')) {
            const mfr  = typeof data.manufacturer === 'string' ? data.manufacturer.trim() : '';
            const code = typeof data.code === 'string' ? data.code.trim() : '';
            if (mfr || code) {
                row.cells.model = modelCell('', mfr || detectSupplier(text), code);
                if (!row.suggested.includes('model')) row.suggested.push('model');
                got.push('model');
                const i = declined.indexOf('model');
                if (i >= 0) declined.splice(i, 1);
            }
        }

        /* A declined field is a finding, not a failure: it means the sheet did
           not state one value, usually because it covers a whole product family.
           The rule pass is told not to paper over it - see pdfIntoRow. */
        row.declined = (row.declined || []).filter(k => !want.includes(k)).concat(declined);
        return got;
    }
};

/* ---- CRI house format ----------------------------------------------------
   A number, or a number with a plus. Nothing else reaches the column, from
   either reader: "Ra80+" and ">90" and "CRI Min. 90" all describe the same
   thing three ways, and a column carrying three notations cannot be sorted,
   filtered or compared. Anything that will not reduce to the house shape is
   dropped rather than passed through, because a blank is honest and "Ra80+"
   in a column of bare numbers is not. */
function normCri(v) {
    let t = stripMd(String(v || '')).toUpperCase();
    /* a range is two claims, not one - that is a blank */
    if (/\d{2,3}\s*(?:-|–|—|TO)\s*\d{2,3}/.test(t)) return '';
    const atLeast = /[+>≥]/.test(t) || /\bMIN/.test(t);
    const m = t.match(/\d{2,3}/);
    if (!m) return '';
    const n = parseInt(m[0], 10);
    if (n < 1 || n > 100) return '';
    return atLeast ? n + '+' : String(n);
}

/* Normalisers by column. Applied wherever a value enters a cell, so the model
   and the regex scanner cannot disagree about notation. */
const FIELD_NORM = { cri: normCri };

function normField(key, v) {
    const f = FIELD_NORM[key];
    return f ? f(v) : v;
}

/* Casing, spacing and thousands separators are noise when comparing what the
   rules read against what the model read. "IP65" and "IP 65" do not disagree. */
function sameValue(a, b) {
    const norm = s => stripMd(String(s || '')).toUpperCase().replace(/[\s,]+/g, '');
    return norm(a) === norm(b);
}

/* Text out of the PDF. Split out from the parsing below so the model can read
   the sheet before the rules do - see scanFileIntoRow. */
async function pdfExtractText(file, onProgress) {
    ...
    for (let p = 1; p <= nPages; p++) {
        ...
        onProgress(18 + Math.round(14 * p / nPages));
    }
    return text;
}

/* The rule pass. `preText` is the text the model has already been shown; when
   it is absent this extracts the text itself and behaves exactly as it used to,
   which is what keeps the tool working with the model switched off. */
/* Read the original file as base64 so it can be sent to the model as a
   document rather than as extracted text. Skipped for anything that is not a
   PDF, and for very large files - past this size the upload costs more than
   the accuracy is worth, and the text path still works. */
const PDF_SEND_LIMIT = 20 * 1024 * 1024;

async function fileAsBase64(file) {
    if (!file || !file.size || file.size > PDF_SEND_LIMIT) return '';
    ...
    } catch { return ''; }   /* fall back to the text path */
}

async function pdfIntoRow(row, file, onProgress, preText) {
    const text = typeof preText === 'string' ? preText : await pdfExtractText(file, onProgress);
    onProgress(78);
    ...
    const filled = [];
    /* Rules run after the model, so a cell that already holds something holds
       the model's answer. Two things follow: never overwrite it, and where the
       rule read the sheet differently, keep the disagreement instead of
       throwing it away - that is the kind of thing that otherwise surfaces
       during construction documentation. */
    const fill = (key, val) => {
        if (!val) return;
        /* one place where every scanned value is normalised to schedule case:
           CAPS, but mm / lm / h / lm/W stay lowercase */
        const normed = normField(key, val);
        if (!String(normed || '').trim()) return;
        const ruleVal = scheduleCase(normed);
        const cur = (row.cells[key] || '').trim();
        const noted = () => {
            row.ruleRead = row.ruleRead || {};
            row.ruleRead[key] = ruleVal;
        };

        /* The model read this sheet and would not commit to a value. That is
           almost always because the sheet offers a choice - four colour
           temperatures, three drive currents, six finishes - and choosing is
           the engineer's job. A regex faced with the same table just takes the
           first number it sees, so letting it fill the blank here would put
           back exactly the guess the model was careful not to make. Leave the
           cell blank and highlighted; keep the rule's reading on the tooltip
           so nothing is thrown away. */
        if (!cur && (row.declined || []).includes(key)) { noted(); return; }

        if (!cur) { row.cells[key] = ruleVal; filled.push(key); return; }

        /* Cell already holds the model's answer. Never overwrite it, but where
           the rule read the sheet differently, keep the disagreement - that is
           the kind of thing that otherwise surfaces during construction docs. */
        if (sameValue(cur, ruleVal)) return;
        if ((row.suggested || []).includes(key)) noted();
    };
...
/* Settle a disagreement between the two readers, and remember the answer.
   Corrections are stored per manufacturer and replayed on the next scan, so
   telling it once about a supplier's quirk is meant to be the last time. */
function openConflictMenu(anchor, row, key, ruleVal) {
    document.querySelectorAll('.conflict-pop').forEach(n => n.remove());

    const modelVal = row.cells[key] || '';
    const shown = v => stripMd(v).replace(/\n/g, ' / ') || '(blank)';
    const supplier = (row.scan && row.scan.supplier) || '';

    const pop = document.createElement('div');
    pop.className = 'menu-pop conflict-pop';

    const settle = (label, value, learnFrom) => {
        const b = document.createElement('button');
        b.className = 'menu-item cp-item';
        b.textContent = label;
        b.title = label;
        b.addEventListener('click', () => {
            row.cells[key] = value;
            if (row.ruleRead) delete row.ruleRead[key];
            /* settled, so no longer a guess */
            if (row.suggested) row.suggested = row.suggested.filter(k => k !== key);
            if (value && learnFrom && Learning.addCorrection(supplier, key, learnFrom, value)) {
                toast('Learned: ' + (supplier || 'this manufacturer') + ' '
                    + (FIELD_LABELS[key] || key) + ' "' + shown(learnFrom)
                    + '" -> "' + shown(value) + '"', 3600);
            }
            pop.remove();
            persist();
    ...
    settle('Correct', modelVal, ruleVal);
    settle('Incorrect - Change to ' + shown(ruleVal), scheduleCase(ruleVal), modelVal);
    settle('Incorrect - Remove', '', '');

    document.body.appendChild(pop);
    const r = anchor.getBoundingClientRect();
    pop.style.top = Math.min(r.bottom + 6, window.innerHeight - pop.offsetHeight - 10) + 'px';
    pop.style.left = Math.max(8, Math.min(r.right - pop.offsetWidth, window.innerWidth - pop.offsetWidth - 10)) + 'px';
...
function renderBody() {
    ...
    state.rows.forEach((row) => {
        /* A scanning row is built exactly like any other row, then decorated
           below. It used to be replaced by one wide cell holding a spinner,
           which meant the row had no columns until the scan finished and the
           whole table jumped when it did. Now the row takes its place in the
           schedule the moment the file lands, and fills in underneath you.
           A row still scanning is always shown, filters notwithstanding. */
        if (!row.loading && !rowVisible(row)) return;
        shown++;
        const tr = document.createElement('tr');
        ...
        COLS.forEach(c => {
            ...
            /* read off the datasheet by the model rather than by a rule */
            if ((row.suggested || []).includes(c.key) && cellText(row, c.key)) {
                td.classList.add('suggested');
                const q = row.aiSrc && row.aiSrc[c.key];
                td.title = q
                    ? 'Read from the datasheet: "' + q + '"'
                    : 'Read from the datasheet - worth a glance';
            }
            /* What the pattern scanner made of this cell, where that is not
               what the cell now shows. Two cases, one store: the cell holds a
               value and the rule disagreed with it, or the cell is blank
               because the model would not commit and the rule's reading is
               not trusted enough to fill it. Neither reading is discarded. */
            const alt = row.ruleRead && row.ruleRead[c.key];
            if (alt) {
                const asRead = stripMd(alt).replace(/\n/g, ' / ');
                if (cellText(row, c.key)) {
                    td.classList.add('conflict');
                    /* Two readers, two answers, and the cell is showing you the
                       model's. The mark is a real button rather than a styled
                       pseudo-element because settling it teaches the tool:
                       whichever way you go is remembered for this manufacturer
                       and applied to the next sheet from them. */
                    const q = document.createElement('button');
                    q.type = 'button';
                    q.className = 'cell-q';
                    /* Set inline as well as in the stylesheet. A button with no
                       rules applied falls back to the browser's own chrome - a
                       grey box with a border - and that is indistinguishable
                       from a styling bug. Inline wins whatever the CSS does. */
                    q.setAttribute('style',
                        ...
                        + 'color:#dd8128;line-height:0;cursor:pointer;-webkit-appearance:none;appearance:none;');
                    q.innerHTML = '<svg class="ic" viewBox="0 0 24 24">'
                        + '<path d="M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h17a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0z"/>'
                        + '<path d="M12 9v4.5"/><path d="M12 17.3h.01"/></svg>';
                    q.title = 'Pattern scanner read "' + asRead + '" - click to settle';
                    q.addEventListener('mousedown', e => e.preventDefault());
                    q.addEventListener('click', e => {
                        e.stopPropagation();
                        openConflictMenu(q, row, c.key, alt);
                    });
                    td.appendChild(q);
                } else {
                    td.title = 'Left blank because the datasheet does not state a '
                        + 'single value. The pattern scanner read "' + asRead
                        + '" - check it against the sheet before using it.';
                }
            }
            ...
            div.addEventListener('blur', () => {
                ...
                if (row.scanned && (row.cells[c.key] || '') !== (div._before || '')) {
                    ...
                    /* you have looked at it - the disagreement is settled */
                    if (row.ruleRead && row.ruleRead[c.key]) {
                        delete row.ruleRead[c.key];
                        td.classList.remove('conflict');
                        td.removeAttribute('title');
                        const btn = td.querySelector('.cell-q');
                        if (btn) btn.remove();
                    }
                }
            });
        ...
        /* still scanning: hold the row open, spinner in the description cell */
        if (row.loading) {
            tr.classList.add('loading-row');
            tr.querySelectorAll('.cell').forEach(c => { c.contentEditable = 'false'; });
            const host = tr.querySelector('td[data-k="description"]') || tr.children[1];
            if (host) {
                /* just the wheel - the filename and the percentage live in the
                   Datasheets tab, and repeating them in the row was noise */
                const cell = host.querySelector('.cell') || host;
                cell.title = 'Scanning ' + row.loading.name;
...
async function scanFileIntoRow(file, targetRow) {
    ...
    const setPct = (pct) => {
        ...
        /* The row shows a spinner and nothing else, so there is nothing in it
           to tick. Progress is shown in the Datasheets tab. */
        renderScanList();
    };
    ...
    try {
        ...
        if (/\.ies$/i.test(file.name)) {
            ...
            /* 1. text out of the PDF */
            const text = await pdfExtractText(file, setPct);

            /* 2. the model reads the whole sheet, before any rule has run.
                  It is given the PDF itself where possible - a datasheet is a
                  laid-out document, and which number belongs to which label is
                  carried by the layout, not by the reading order. Everything it
                  declines to answer it simply leaves blank. */
            setPct(32);
            const pdfB64 = await fileAsBase64(file);
            setPct(38);
            const read = await LLM.extract(row, text, pdfB64);

            /* 3. the rules backfill those blanks, and flag anywhere they read
                  the sheet differently to the model */
            setPct(70);
            const byRule = await pdfIntoRow(row, file, setPct, text);
            filled = read.concat(byRule.filter(k => !read.includes(k)));
        }
        await tick(200);
        row.scanned = true;
        row.loading = null;
        ...
        /* An IES file carries photometry, not specifications. If a datasheet
           was scanned into this row earlier its text is still stored, so the
           model can still fill what the IES cannot. */
        if (missing.length && /\.ies$/i.test(file.name)) {
            let srcText = '';
        ...
        renderScanList();
        autosizeAll();
        const clashes = Object.keys(row.ruleRead || {})
            .filter(k => cellText(row, k)).length;
        if (clashes) {
            toast(`${file.name}: ${filled.length} field${filled.length === 1 ? '' : 's'} filled, ${clashes} where the pattern scanner read it differently - check the outlined cells`, 4600);
        } else if (missing.length && filled.length) {
...
/* Stamp the running build into the footer. Four rounds were spent this week
   arguing about whether a change had landed, when the browser was holding a
   stylesheet two versions old. Now the page says which one it has. */
function stampBuild() {
    const src = [...document.scripts].map(s => s.src).find(u => /lfs\.js/.test(u)) || '';
    const ver = (src.match(/[?&]v=([\d.]+)/) || [, '?'])[1];
    const css = [...document.styleSheets].some(ss => {
        try { return [...ss.cssRules].some(r => /content\s*:\s*["']AI/i.test(r.cssText)); }
        catch { return false; }
    });
    const el = document.querySelector('.site-footer p');
    if (el) el.textContent = 'Light Fitting Schedule build ' + ver
        + (css ? '  |  STALE STYLESHEET - hard refresh' : '')
        + '  |  Plexus Tools | Plexus Engineers';
}
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
        LLM, IES, buildWorkbook, COLS, blankRow, iesIntoRow, pdfIntoRow, pdfExtractText, sameValue,
        fileAsBase64, modelCell, normCri, normField, mdToRich, stripMd,
